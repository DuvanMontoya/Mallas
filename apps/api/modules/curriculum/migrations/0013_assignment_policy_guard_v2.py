...
DECLARE
    ...
    target_content_hash varchar(128);
    target_source_set_hash varchar(128);
BEGIN
    IF TG_OP = 'DELETE' THEN
        ...
        RETURN OLD;
    END IF;
    IF OLD.status NOT IN ('PUBLISHED', 'SUPERSEDED', 'RETIRED') AND NEW.status = 'PUBLISHED' THEN
        ...
        IF publication_capability IS DISTINCT FROM 'allowed' THEN
            RAISE EXCEPTION 'Assignment policies can only be published through governance';
        END IF;
        ...
        IF NEW.content_hash = '' OR NEW.source_set_hash = '' OR NEW.published_at IS NULL
           OR NEW.prepared_by_id IS NULL OR NEW.approved_by_id IS NULL
           OR NEW.prepared_by_id = NEW.approved_by_id
           OR target_status NOT IN ('PUBLISHED', 'SUPERSEDED', 'RETIRED')
           OR (target_status = 'RETIRED' AND NOT NEW.allow_retired_revision)
           OR target_content_hash = '' OR target_source_set_hash = ''
           OR sealed_evidence_count < 1 THEN
            RAISE EXCEPTION 'Published assignment policy is missing governed material';
        END IF;
    END IF;
    IF OLD.status IN ('PUBLISHED', 'SUPERSEDED', 'RETIRED') THEN
...
def restore_previous_guard(apps: object, schema_editor: object) -> None:
    del apps
    connection = schema_editor.connection  # type: ignore[attr-defined]
    if connection.vendor == "postgresql":
        previous_sql = import_module(
            "modules.curriculum.migrations.0010_assignment_policy_publication_guard"
        ).FORWARD_SQL
        with connection.cursor() as cursor:
...
class Migration(migrations.Migration):
    dependencies = [("curriculum", "0012_curriculumassignmentpolicy_allow_retired_revision")]
    operations = [migrations.RunPython(install_guard, restore_previous_guard)]
