...
export function StudentOnboardingForm({
  ...
  const [historyStatus, setHistoryStatus] = useState(
    initial.history_step_status === "PENDING" ? "SKIPPED" : initial.history_step_status,
  );
  ...
  const [tourStatus, setTourStatus] = useState(
    initial.tour_status === "PENDING" ? "COMPLETED" : initial.tour_status,
  );
  ...
  function save(complete: boolean) {
    ...
    startTransition(async () => {
      ...
      if (result.data.completed) {
        router.replace(assignmentResolved ? "/curriculum" : "/");
        router.refresh();
      }
    });
  }
  ...
  return (
    <div className="onboarding-workspace">
      ...
      <form className="panel onboarding-checklist" onSubmit={(event) => { event.preventDefault(); save(true); }}>
        ...
        <fieldset><legend>2. Decide cómo iniciar tu historia</legend><label className="field-group"><span>Historia académica</span><select value={historyStatus} onChange={(event) => setHistoryStatus(event.target.value)}><option value="SKIPPED">La completaré después</option><option value="IMPORTED">Ya fue importada o registrada</option></select><small>Omitirla no bloquea la malla; sólo deja los estados personales sin calcular.</small></label><Link className="text-link" href="/history">Revisar historia</Link></fieldset>
        <fieldset><legend>3. Elige tu período actual</legend><label className="field-group"><span>Período</span><select required value={termId} onChange={(event) => setTermId(event.target.value)}>{terms.map((term) => <option key={term.id} value={term.id}>{term.code} · {term.status}</option>)}</select></label></fieldset>
        ...
        <fieldset><legend>5. Recorrido visual</legend><label className="field-group"><span>Estado del recorrido</span><select value={tourStatus} onChange={(event) => setTourStatus(event.target.value)}><option value="COMPLETED">Entendí la leyenda básica</option><option value="SKIPPED">Prefiero verlo después</option></select><small>Podrás abrir la ayuda de la malla cuando quieras.</small></label></fieldset>
        {error ? <Alert tone="error">{error}</Alert> : null}
        <div className="form-actions"><button className="button button-secondary" type="button" disabled={pending || !termId} onClick={() => save(false)}>Guardar y continuar después</button><button className="button button-primary" type="submit" disabled={pending || !termId || !identityConfirmed}>{pending ? "Guardando…" : "Terminar y abrir mi espacio"}</button></div>
      </form>
    </div>
  );
}
